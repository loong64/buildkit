...
FROM ghcr.io/loong64/minio/minio:${MINIO_VERSION} AS minio
FROM ghcr.io/loong64/minio/mc:${MINIO_MC_VERSION} AS minio-mc

# xx is a helper for cross-compilation
FROM --platform=$BUILDPLATFORM ghcr.io/loong64/tonistiigi/xx:${XX_VERSION} AS xx

# golatest is alias for Go base image
FROM --platform=$BUILDPLATFORM ghcr.io/loong64/golang:${GO_VERSION}-alpine${ALPINE_VERSION} AS golatest
...
FROM --platform=$BUILDPLATFORM ghcr.io/loong64/alpine:${ALPINE_VERSION} AS cni-plugins
...
ARG TARGETPLATFORM
WORKDIR /opt/cni/bin
RUN curl -fsSL https://github.com/loong64/plugins/releases/download/${CNI_VERSION}/cni-plugins-${TARGETOS}-${TARGETARCH}-${CNI_VERSION}.tgz | tar xzv
...
COPY --link --from=ghcr.io/loong64/tonistiigi/binfmt:buildkit-v9.2.2-6@sha256:1c243e5d9a728b623efaaa2a8efca04937760bfc6efba1f1be95de04b5721ae4 / /out/
WORKDIR /out/
...
FROM --platform=$BUILDPLATFORM ghcr.io/loong64/alpine:${ALPINE_VERSION} AS releaser
...
ARG TARGETARCH
SHELL ["/bin/bash", "-c"]
RUN wget https://github.com/loong64/nydus/releases/download/$NYDUS_VERSION/nydus-static-$NYDUS_VERSION-$TARGETOS-$TARGETARCH.tgz
...
FROM ghcr.io/loong64/alpine:${ALPINE_VERSION} AS rootless
